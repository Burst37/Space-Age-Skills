...
export async function POST(req: Request) {
  ...
  const base = (artDirected || "Make a cleaner, higher-quality version of this thumbnail.") + (singleImage ? SINGLE : "");
  ...
  const jobs = prompts.map(async (prompt, i): Promise<Buffer> => {
    ...
    // YouTube needs 16:9 but gpt-image-2 only renders 3:2. Handed a 16:9 ref, the
    // model fills its narrower 3:2 frame by height and CROPS THE LEFT/RIGHT EDGES
    // off the design before we ever see it. `preserve` letterboxes the ref up to
    // 3:2 first (full design centred → sides safe), then trims the top/bottom
    // bands back to 16:9. Full width, no side clip, no blurred bars. 1920x1080 PNG.
    a.push("--out", sub, "--slug", "thumb", "--fit", "preserve", "--out-size", "1920x1080", "--format", "png");
