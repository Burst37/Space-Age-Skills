...
function glmKey(): string | null {
  ...
  try {
    const env = fs.readFileSync(path.join(os.homedir(), ".hermes", "profiles", "glm-5-2", ".env"), "utf8");
...
export async function POST(req: Request) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      const messages = [
        { role: "system", content: "You are GLM-5.2, Zhipu's frontier coding model, running inside Julian's Agent OS. Be concise and direct. When asked to build something, return complete, self-contained code." },
