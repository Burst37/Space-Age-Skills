...
function fileUrl(rel: string) { return `/api/thumbnails/file?path=${encodeURIComponent(rel)}`; }
                ...
                <div className="flex gap-2 overflow-x-auto pb-1">
                  {(s.inputFiles || []).map((inp) => (
                    <div key={inp} className="shrink-0 relative">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={fileUrl(inp)} alt="reference" className="h-20 aspect-video object-cover rounded border cursor-zoom-in" style={{ borderColor: "var(--line-soft)", opacity: 0.65 }} onClick={() => setEnlarged(fileUrl(inp))} />
                      <span className="absolute top-1 left-1 text-[8px] px-1 rounded" style={{ background: "rgba(0,0,0,0.6)", color: "#fff" }}>REF</span>
                    </div>
                  ))}
                  {s.outputs.map((o) => (
                    /* eslint-disable-next-line @next/next/no-img-element */
                    <img key={o} src={fileUrl(o)} alt="version" className="h-20 aspect-video object-cover rounded border shrink-0 cursor-zoom-in" style={{ borderColor: `${ACCENT}44` }} onClick={() => setEnlarged(fileUrl(o))} />
                  ))}
                </div>
              </div>
            ))}
