...
export function roomAgents(): RoomAgent[] {
  return ROOM_AGENTS.map((a) => a.provider === "ollama" ? { ...a, model: localModel() } : a);
}
...
function openRouterKey(): string | null {
  ...
  if (!existsSync(f)) return process.env.OPENROUTER_API_KEY?.trim() || null;
  try {
    const line = readFileSync(f, "utf8").split("\n").find((l) => l.startsWith("OPENROUTER_API_KEY="));
    if (line) return line.slice("OPENROUTER_API_KEY=".length).replace(/^["']|["']$/g, "").trim() || null;
  } catch {}
...
const ROOM_SYSTEM =
  "You are in a fast, live group chat with the user (Julian) and a few other AI agents. " +
  ...
  "You can take REAL actions in Julian's vault, but ONLY when he clearly asks for it:\n" +
  "• To SAVE your point as a note in his vault, add a final line exactly: NOTE:: <a short title>\n" +
  "• To ADD an idea to his project pipeline, add a final line exactly: PIPELINE:: <one-line idea>\n" +
...
async function orComplete(model: string, sys: string, user: string, key: string, signal?: AbortSignal): Promise<string> {
  const r = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    body: JSON.stringify({ model, max_tokens: 1200, temperature: 0.75, messages: [{ role: "system", content: sys }, { role: "user", content: user }] }),
  });
...
// Pull REAL context from Julian's Obsidian vault so the agents answer about HIS
// world (his business, projects, notes, memories) — not with generic advice.
...
export async function roomContext(query: string): Promise<{ text: string; sources: RoomSource[] }> {
  ...
  try {
    const a = await readNote("04 Resources/About Julian.md");
    if (a?.content) { parts.push("WHO JULIAN IS:\n" + a.content.replace(/^---[\s\S]*?---/, "").replace(/[#*>[\]]/g, "").replace(/\n{2,}/g, "\n").trim().slice(0, 1100)); sources.push({ kind: "profile", title: "About Julian" }); }
  } catch {}
  try {
    const hits = await searchNotes(query, 5);
    if (hits.length) { parts.push("RELEVANT NOTES FROM HIS VAULT:\n" + hits.map((h) => `• ${h.title} — ${(h.preview || "").replace(/\s+/g, " ").slice(0, 150)}`).join("\n")); hits.forEach((h) => sources.push({ kind: "note", title: h.title })); }
  } catch {}
  try {
    const mem = await searchOmi(query, 6);
    if (mem.length) { parts.push("RELEVANT MEMORIES (things he's said/done):\n" + mem.map((m) => "• " + m.slice(0, 170)).join("\n")); sources.push({ kind: "memory", title: `${mem.length} memories` }); }
  } catch {}
  try {
    const rec = await recentNotes(8);
    if (rec.length) parts.push("WHAT HE'S WORKING ON LATELY: " + rec.map((r) => r.title).join(", "));
  } catch {}
...
// One agent's reply, given the transcript + Julian's vault context. Cloud agents
...
export async function roomReply(agent: RoomAgent, transcript: RoomTurn[], context: string, signal?: AbortSignal): Promise<string> {
  const ctx = context
    ? `\n\n--- JULIAN'S REAL CONTEXT (from his Obsidian vault) ---\n${context}\n--- end context ---\nGround your reply in THIS. Reference his actual business, projects, and notes. Be specific to Julian — never give generic advice you could give anyone.`
    : "";
  ...
  try { return await orComplete(agent.model, sys, user, key, signal); }
  catch (e) {
