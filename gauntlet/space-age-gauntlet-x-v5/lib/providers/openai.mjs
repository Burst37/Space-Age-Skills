...
export async function callOpenAI({prompt,system="",model,images=[],maxOutputTokens,apiKey=process.env.OPENAI_API_KEY}){
  if(!apiKey) throw new Error("OPENAI_API_KEY missing");
  const content=[{type:"input_text",text:prompt},...images.map(url=>({type:"input_image",image_url:url,detail:"high"}))];
  const body={model:model||process.env.OPENAI_MODEL||"gpt-5.6",input:[
    ...(system?[{role:"system",content:[{type:"input_text",text:system}]}]:[]),
    {role:"user",content}
  ]};
  if(maxOutputTokens) body.max_output_tokens=maxOutputTokens;
  const data=await checkedJson(await fetch("https://api.openai.com/v1/responses",{
