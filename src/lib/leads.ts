...
//
// Provider-pluggable: swap the data engine by adding a key, never editing code:
//   • CSV / domains + Hunter.io  → the cheap ($0 free tier) default
//   • Apollo.io                  → flip-on when APOLLO_API_KEY is set (paid)
// Personalisation runs on the same OpenRouter key the rest of the OS uses.
...
const SEEN_FILE = path.join(AOS, "leads.json");
const HISTORY_FILE = path.join(AOS, "leads-history.json");
