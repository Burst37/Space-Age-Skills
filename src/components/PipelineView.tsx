...
import { Inbox, Sparkles, ShieldCheck, Cpu, CheckCircle2, Send, Loader2, X, Check, Ban, ArrowRight, FileText, Play, ExternalLink, Square, Star, LayoutGrid, Columns3 } from "lucide-react";
...
export default function PipelineView() {
  ...
  return (
    <div className="relative">
              ...
              <div key={col.key} className="flex-1 min-w-[230px] relative">
                ...
                <div className="px-2">
                  ...
                  <div className="space-y-2.5">
                    ...
                    {list.map((it) => (
                      <Card key={it.slug} it={it} busy={busy[it.slug]} onOpen={() => setSelected(it)} onShape={() => shape(it.slug)} onDecide={(a) => decide(it.slug, a)} onBuild={() => build(it.slug)} onStop={() => stop(it.slug)} onPin={() => pin(it.slug, !it.pinned)} />
                    ))}
                  </div>
                </div>
              </div>
      ...
      {/* Detail drawer */}
      <AnimatePresence>
        {selected && <Drawer it={selected} busy={busy[selected.slug]} onClose={() => setSelected(null)} onShape={() => shape(selected.slug)} onDecide={(a) => decide(selected.slug, a)} onBuild={() => build(selected.slug)} onStop={() => stop(selected.slug)} />}
      </AnimatePresence>
      </div>
    </div>
  );
...
function Card({ it, busy, onOpen, onShape, onDecide, onBuild, onStop, onPin }: { it: Item; busy?: string; onOpen: () => void; onShape: () => void; onDecide: (a: boolean) => void; onBuild: () => void; onStop: () => void; onPin: () => void }) {
  const accent = it.route ? ROUTE_COLOR[it.route] : "#22d3ee";
  return (
    <motion.div layout initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}
      className="rounded-xl border p-3 cursor-pointer transition hover:border-[var(--panel-border-hot)]" style={{ borderColor: it.pinned ? "rgba(251,191,36,0.5)" : "var(--panel-border)", background: "rgba(255,255,255,0.015)", borderLeft: `2px solid ${accent}` }}
      onClick={onOpen}>
      <div className="flex items-start justify-between gap-2">
        <div className="text-[12.5px] font-medium text-[var(--fg)] leading-snug">{it.title}</div>
        <button onClick={(e) => { e.stopPropagation(); onPin(); }} title={it.pinned ? "Unpin — remove from top" : "Pin to top — show this first"} className="shrink-0 -mt-0.5 -mr-0.5 p-1 rounded-md hover:bg-[var(--bg-mid)]">
...
function Drawer({ it, busy, onClose, onShape, onDecide, onBuild, onStop }: { it: Item; busy?: string; onClose: () => void; onShape: () => void; onDecide: (a: boolean) => void; onBuild: () => void; onStop: () => void }) {
  return (
