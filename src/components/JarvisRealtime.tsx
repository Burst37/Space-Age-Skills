...
import { Loader2, Send, X, Radio, History } from "lucide-react";
...
export default function JarvisRealtime({ voice = "ash", onClose }: { voice?: string; onClose?: () => void }) {
  ...
  // The butler called a tool — execute it, then hand the result back so it can confirm out loud.
  async function handleFunctionCall(ev: { call_id?: string; arguments?: string }) {
    const dc = dcRef.current; if (!dc) return;
    let target = "";
    try { target = JSON.parse(ev.arguments || "{}").target || ""; } catch { /* ignore */ }
    ...
    if (dc.readyState === "open") {
      dc.send(JSON.stringify({ type: "conversation.item.create", item: { type: "function_call_output", call_id: ev.call_id, output: JSON.stringify({ success: ok, target }) } }));
  ...
  const disconnect = useCallback(() => { cleanup(); setStatus("idle"); setSpeaking(false); }, []);
  useEffect(() => () => cleanup(), []);
