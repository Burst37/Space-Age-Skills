...
- **`style`** → component visual treatment.
...
6. **Fix imports in third-party components** — After adding components from community registries, check the added non-UI files for hardcoded import paths like `@/components/ui/...` and rewrite to the project aliases.
7. **Review added components** — After adding a component or block from any registry, always read the added files and verify they are correct.
...
**Templates:** `next`, `vite`, `start`, `react-router`, `astro` and `laravel`.
**Preset codes:** Version-prefixed base62 strings, from ui.shadcn.com.
