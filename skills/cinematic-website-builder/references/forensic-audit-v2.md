...
| A14 | S2 | **Stale stack.** GSAP pinned to 3.12.5. Since 3.13 every plugin is free (SplitText, ScrambleText, DrawSVG, Flip, Inertia, ScrollSmoother) — v2 hand-rolls weaker versions (scramble, SVG draw, "odometer" count-up). No native-first primitives (`animation-timeline: view()`, View Transitions, `@property`, `interpolate-size`, scroll-snap). | Tech Stack | GSAP 3.15.0 + plugins loaded per module; native CSS first (stack router in CINEMATIC_WEBSITE_BUILDER.md). |
