...
This skill ships as two identical files: **`cli-anything.md`** is the canonical,
self-identifying copy — carry that one to any other platform. `SKILL.md` is the
same content at the filename Claude Code's loader and agent-core's
`discoverSkills()` both require. Edit `cli-anything.md`, then copy it over
`SKILL.md`; never let the two drift.
