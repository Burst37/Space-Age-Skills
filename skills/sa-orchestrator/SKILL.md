...
description: >
  Universal capability reasoning engine and meta-skill for Space Age AI Solutions.
  Runs on every message: scans available skills, MCPs, and CLIs to build a live
  capability map, extracts intent signals from the input, scores and ranks matches,
  and routes to the best skill/MCP/CLI or chain of them without any hardcoded tool
  names. Asks at most 2 clarifying questions when genuinely ambiguous, otherwise
  executes silently. Highest priority, load order 0 — active on every turn.
class: meta-skill
priority: highest
license: MIT
---
...
Skill: ~/.claude/skills/sa-orchestrator/SKILL.md  
